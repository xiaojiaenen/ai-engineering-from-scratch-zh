...
**方式 A — 阅读。** 在 浏览器 上打开任何已完成的课程，或在[目录](#contents)下展开一个阶段。无需设置，无需克隆。
...
**方式 C — 找到你的水平 *(推荐)*。** 智能跳过。在 Claude、Cursor、Codex、OpenClaw、Hermes 或任何安装了课程技能的智能体中：

```bash
/find-your-level
```

十个问题。将你的知识映射到起始阶段，并构建一个包含时间估算的个性化路径。每个阶段结束后：
...
| 为你的团队或学校复刻项目 | [FORKING.md](FORKING.md) |
...
当任何规则失败时，退出码为非零。规则（L001–L010）验证目录结构、`docs/en.md` 的存在性与 H1 标题、`code/` 的非空性、`quiz.json` 的架构（拒绝了导致 issue #102 的遗留 `q/choices/answer` 键），以及课程文档内部的相对链接。
...
免费，MIT 许可，435 节课程。课程仅靠赞助维持。仅接受现金。

**影响力（截至2026-05-14验证）：** 55,593 月访客 · 90,709 页面浏览量 · 7.5K 星标 ·
Twitter/X 是第一大获客渠道。

| 等级 | 每月费用 | 您将获得 |
|------|------|---|
| 支持者 | $25 | 名字列入 BACKERS.md |
| 铜牌 | $250 | README 赞助商区块中出现纯文本行 + 发布日推文 |
| 银牌 | $750 | README 中的小型标志 + 在 API 课程中列为支持的提供商之一 |
| 金牌 | $2,000 | README 中的中型标志 + 赞助商页面 + 每季度 X / LinkedIn 联合推荐 |
| 白金 | $5,000 | 页面顶部的醒目标志 + 一节专门的集成课程，最多1位合作伙伴 |

完整的价目表、硬性规则、定价锚点和影响力数据：[SPONSORS.md](SPONSORS.md)。
...
<a href="https://star-history.com/#xiaojiaenen/ai-engineering-from-scratch-zh&Date">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=xiaojiaenen/ai-engineering-from-scratch-zh&type=Date&theme=dark">
    <img alt="星标历史" src="https://api.star-history.com/svg?repos=xiaojiaenen/ai-engineering-from-scratch-zh&type=Date" width="100%">
  </picture>
</a>

如果这份手册对你有帮助，请为仓库点亮星标。它让项目得以延续。
